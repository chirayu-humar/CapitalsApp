...
class AppStore extends Component {
  ...
  onChangeInput = event => {
    ...
    this.setState(prevState => ({
      selectedTabId: prevState.selectedTabId,
      searchInput: inputValue,
    }))
  }

  render() {
    const {selectedTabId, searchInput} = this.state
    const appsListUpdated = appsList.filter(
      eachItem => eachItem.category === selectedTabId,
    )
    const appsListUpdated2 = appsListUpdated.filter(eachItem =>
